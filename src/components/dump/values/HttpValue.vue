  ...
  <div class="--code flex flex-col space-y-1">
    ...
    <div class="ml-2.5">
      <div class="flex flex-row items-center space-x-3">
        ...
        <span class="text-[11px] tracking-tighter font-bold" :class="responseStatusColor">{{
          response.status.value
        }}</span>
        <span class="-status-value">Total Time: {{ response.handlerStats.value.total_time.value }}</span>
        <span class="-status-value"
          >Primary IP: {{ response.handlerStats.value.primary_ip.value }}:{{
            response.handlerStats.value.primary_port.value
          }}</span
        >
      </div>
    </div>
    ...
    <div class="--code !mt-2">
      <component
        ...
        :capsule-dto="props.messageDto.data"
      ></component>
    </div>
  </div>
</template>
