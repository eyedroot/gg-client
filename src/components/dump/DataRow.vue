<template>
  <div
    v-if="['log', 'throwable', 'http.request', 'sql.model'].includes(messageDto.type)"
  ...
  >
    <div class="--code-wrap flex-auto items-center relative" ref="code">
      ...
      <div
        v-if="messageDto.type.startsWith('log')"
        class="--code"
        :class="{ throwable: messageDto.type === 'throwable' }"
      >
      ...
      <ThrowableValue v-else-if="messageDto.type === 'throwable'" :message-dto="messageDto">
        <RowExtensions
          :is-local-data="isLocalData"
          :created-at="messageDto?.__createdAt"
          @saveToLocalStorage="saveToLocalStorage"
      ...
      <HttpValue v-else-if="messageDto.type === 'http.request'" :message-dto="messageDto" :display-id="displayId">
      </HttpValue>

      <SqlValue v-else-if="messageDto.type === 'sql.model'" :message-dto="messageDto" :display-id="displayId">
      </SqlValue>

      <div v-if="showBacktrace">
        <BackTrace :backtrace="this.messageDto.trace" :focus-file="focusFile()"> </BackTrace>
      </div>
    </div>
  </div>

  <UsageValue v-else-if="messageDto.type === 'log.usage'" @removeItem="removeItem(id)" :message-dto="this.messageDto">
  </UsageValue>
...
export default {
  ...
  methods: {
    ...
    focusFile() {
      if (this.messageDto.language.toUpperCase() === 'PHP') {
        ...
        for (let i = 0; i < this.messageDto.trace.length; i++) {
          ...
          if (
            row.file &&
            !row.file.includes('gg/src/Gg.php') &&
            !row.file.includes('gg/src/helpers/helper.php') &&
            !row.file.includes('/vendor/')
          ) {
            return row;
          }
        }
    ...
    copyJson() {
      ...
      clipboardFromString(JSON.stringify(result, null, 2), () => this.$emit('showBalloon', 'Copied Json'));
    },
    copyAssocArray() {
