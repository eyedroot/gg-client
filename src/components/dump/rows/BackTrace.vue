<template>
  <div class="--backtrace flex flex-col px-1.5 py-3 overflow-x-auto">
    <div v-if="backtrace.length === 0">
      <div class="text-gray-600 text-center dark:text-gray-300">
        No backtrace available. <code>\gtrace($data);</code>
      </div>
    </div>

    <div :key="index" v-for="(row, index) in backtrace" class="flex flex-col">
      <div
        class="flex flex-row items-center text-gray-600 hover:text-gray-900 hover:bg-gray-100 pl-2 leading-7 dark:hover:bg-gray-800"
      >
        <button
          class="inline-flex items-center justify-center w-4 h-4 bg-gray-300 hover:bg-gray-500 mr-1.5 text-white cursor-pointer select-none rounded dark:bg-gray-900"
          @click="toggleCodes(index)"
        >
          {{ showCodesIndexes.includes(index) ? '-' : '+' }}
        </button>

        <span
          class="text-[14px] text-gray-900 dark:text-gray-400"
          :class="{ 'text-gray-700 font-bold': row.file === this.focusFile.file }"
        >
          {{ row.file ? row.file : row.class }}:<strong>{{ row.line }}</strong>
        </span>

        <code class="ml-1.5 text-blue-600 tracking-tight font-bold dark:text-blue-400">
          {{ row?.function }}(<span
            v-if="row?.args?.type !== 'NULL'"
            class="inline-flex bg-gray-300 rounded-sm px-0.5 cursor-pointer select-none min-w-[23.88px] items-center justify-center text-gray-700 dark:bg-gray-600 dark:text-gray-300"
            @click="toggleParameters(index)"
            >{{ showParametersIndexes.includes(index) ? '▼' : '...' }}</span
          >)
        </code>
      </div>
      ...
      <CodeViewer
        v-if="showCodesIndexes.includes(index)"
