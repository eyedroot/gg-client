  ...
  <div class="--code viewer">
    <div class="flex flex-col">
      <div v-for="(row, idx) in sourceCode" :key="idx" class="flex flex-row">
        ...
        <code
          class="flex-1 whitespace-pre text-gray-700 dark:text-gray-200"
          :class="{ 'bg-gray-200 dark:bg-gray-900': row.line === focusLine }"
          v-html="highlightCode(row.code)"
        ></code>
      </div>
    </div>
  </div>
  ...
  setup() {
    const highlightCode = (code) => {
      return code
        ...
        .replace(
          /\s(foreach|echo|class|function|interface|trait|while|do|for|break;|continue|try|catch|finally|throw|return|yield|use|namespace|global|isset|empty|include|include_once|require|require_once|print|unset|list|clone|declare|instanceof|and|or|xor|new|as|bool|boolean|int|integer|float|double|string|array|object|void|mixed|numeric|callback|callable|false|true|null|if|else|elseif|else if)\s/g,
